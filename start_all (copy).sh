...
kill_pi_process() {
  ...
  ssh chengjindu@"$(jq -r '.pi_ip' start_config.json)" 'tmux send-keys -t ros_session C-c'
  ...
  ssh chengjindu@"$(jq -r '.pi_ip' start_config.json)" 'tmux kill-session -t ros_session'
}
...
ssh chengjindu@"$(jq -r '.pi_ip' start_config.json)" 'tmux new-session -d -s ros_session "bash /home/chengjindu/SoftMag/Console/ros_workspace/start_console_pi.sh"' &
sleep 2  # Give some time for tmux to start the session

# Verify tmux session on Raspberry Pi
ssh chengjindu@"$(jq -r '.pi_ip' start_config.json)" 'tmux list-sessions'
