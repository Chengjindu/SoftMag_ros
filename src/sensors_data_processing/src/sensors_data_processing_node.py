...
def initialize_sensor_filters(sensor_ids, sampling_freq=sampling_frequency, passband_freq=passband_frequency, order=order, passband_ripple=passband_ripple):
    global sensor_filters, delay
    ...
    for sensor_id in sensor_ids:
        ...
        sensor_filters[sensor_id] = {
            'b': b, 'a': a,
            'zi_x': zi * np.zeros(len(b) - 1),
            'zi_y': zi * np.zeros(len(b) - 1),
            'zi_z': zi * np.zeros(len(b) - 1)
        }
    return delay, sensor_filters

def filter_sensor(data, filter_config):     # Function to apply filter to the data
    try:
        xMag, yMag, zMag = data
        ...
        rospy.logerr("Error filtering sensor data: %s", e)
        return 0.0, 0.0, 0.0
...
def recalibrate_sensors():  # Recalibrate all sensors
    ...
    # Reinitialize sensor_filters with the measured sampling frequency
    initialize_sensor_filters(sensor_ids, sampling_freq=sampling_frequency)
...
def zero_sensor(sensor_id):  # Function for offset elimination
    global initilization_window, initial_values, data_stablized, sensor_data, sensor_filters, delay
    ...
    while True:
        ...
        while Readout_count < initilization_window and not rospy.is_shutdown():
            ...
            if data is not None and len(data) == 3:
                ...
                try:
                    filtered_x, filtered_y, filtered_z = filter_sensor(data, sensor_filters[sensor_id])
...
def process_sensor_data(sensor_id, publisher):      # Process data for a single sensor
    global sensor_data, sensor_filters, delay, initial_values, data_stablized, processing_count, publishing_count, start_time
...
if __name__ == '__main__':
    ...
    # Initialize sensor_filters with the measured sampling frequency
    initialize_sensor_filters(sensor_ids, sampling_freq=sampling_frequency)

    sensors_publishers = {sensor_id: rospy.Publisher(f'processed_sensor_data_{sensor_id}', Float32MultiArray, queue_size=10) for sensor_id in sensor_ids}
    ...
    with ThreadPoolExecutor(max_workers=len(sensor_ids)) as executor:
        for sensor_id in sensor_ids:
            executor.submit(process_sensor_data, sensor_id, sensors_publishers[sensor_id])
