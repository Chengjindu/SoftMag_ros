...
def initialize_filters(sensor_ids, sampling_freq=sampling_frequency, passband_freq=passband_frequency, order=order, passband_ripple=passband_ripple):
    ...
    # Higher order sensor_filters have a steeper drop-off but can introduce more delay and potential ringing artifacts
...
def zero_sensor(sensor_id):     # Function for offset elimination
    global initilization_window, initial_values, data_stablized, sensor_data, sensor_filters, delay, contact_status
...
def process_sensor_data(sensor_id, publisher):      # Process data for a single sensor
    global sensor_data, sensor_filters, delay, initial_values, data_stablized, contact_detected, processing_count, publishing_count, start_time
