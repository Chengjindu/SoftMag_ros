...
class WholeGripperController(Sofa.Core.Controller):
    ...
    def __init__(self, *a, **kw):
        ...
        for i in range(1,3):
            ...
            # fingerNode = self.node.getChild('finger' + str(i))
            # if not fingerNode:
            #     print(f"Error: finger {i} node not found.")
            ...
            #
            # surfacePressureConstraintNode = fingerNode.cavity.getObject('SurfacePressureConstraint')
            # if not surfacePressureConstraintNode:
            #     print(f"Error: SurfacePressureConstraint node not found in finger {i}.")
            #
            # self.constraints.append(surfacePressureConstraintNode)
